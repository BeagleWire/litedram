...
static inline uint16_t sdram_dfii_pi0_wrdata_read(void) {
	uint16_t r = csr_read_simple(CSR_BASE + 0x818L);
	r <<= 8;
	r |= csr_read_simple(CSR_BASE + 0x81cL);
	return r;
}
static inline void sdram_dfii_pi0_wrdata_write(uint16_t v) {
	csr_write_simple(v >> 8, CSR_BASE + 0x818L);
	csr_write_simple(v, CSR_BASE + 0x81cL);
}
#define CSR_SDRAM_DFII_PI0_RDDATA_ADDR (CSR_BASE + 0x820L)
#define CSR_SDRAM_DFII_PI0_RDDATA_SIZE 2
static inline uint16_t sdram_dfii_pi0_rddata_read(void) {
	uint16_t r = csr_read_simple(CSR_BASE + 0x820L);
	r <<= 8;
